...
void* _DynArray_Create(U64 elementSize, U64 elementCount) {
	...
	void* dynArray = Allocate(totalSize, MemoryTag_DynamicArray);
	if (dynArray == NULL) { return NULL; }

	// Zero-initialize all of the elements for convenience and safety.
	MemZero(dynArray, totalSize);
...
B8 _DynArray_Trim(DynArrayT dynArray) {
	...
	DynArrayMetadata* newMeta = Reallocate(meta, totalSize, MemoryTag_DynamicArray);
...
void _DynArray_Insert(DynArrayT dynArray, U64 index, const void* element) {
	...
	if (index <= (meta->Size - 1)) {
		...
		MemMove(newPosition, oldPosition, bytesToMove);  // Memory Move is required as the two blocks overlap
	}

	// Finally, copy in our new value.
	void* ptr = (*dynArray) + (meta->Stride * index);
	MemCopy(ptr, element, meta->Stride);
...
void _DynArray_Extract(DynArrayT dynArray, U64 index, void* element) {
	...
	if (element != NULL) {
		...
		MemCopy(element, ptr, meta->Stride);
	}
